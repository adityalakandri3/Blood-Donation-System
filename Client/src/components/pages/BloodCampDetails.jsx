...
import { useGetBloodCampById, useUserRegistrationCamp } from '../../hooks/react-query/query-hooks/bloodCamp';
...
const BloodCampDetails = () => {
  const { id } = useParams();
  const { data, isLoading, isError } = useGetBloodCampById(id);
  console.log(data)
  const {mutate} = useUserRegistrationCamp();
      ...
      >
        ...
        >
          <CardMedia
            component="img"
            height="250"
            image={`http://localhost:3006/${camp.image.replace(
                /\\/g,
                "/"
              )}`}
            alt="Blood Camp"
